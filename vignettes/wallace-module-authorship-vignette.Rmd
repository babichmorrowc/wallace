...
```{r}
layerStats <- function (x, stat = "pearson", w, asSample = TRUE, na.rm = FALSE, shinyLogs = NULL) 
{
    stat <- tolower(stat)
            ...
            for (j in i:nl) {
                ...
                if (na.rm) {
                  v <- cellStats(r, stat = "sum", na.rm = na.rm)/(n - 
                    cellStats(r, stat = "countNA") - asSample)
                }
                else {
                  v <- cellStats(r, stat = "sum", na.rm = na.rm)/(n - 
                    asSample)
...
runCorr_UI <- function(id) {
  ...
  tagList(
    tags$div(title="Calculate pairwise Pearson's Correlation matrix for environmental predictors",
             numericInput(ns("PearCorr"), "Calculate Pairwise Correlations", value = 1))
  )
}
...
runCorr_MOD <- function(input, output, session) {
  reactive({
    for(sp in spIn()) {
      ...
      mod.Corr <- layerStats(spp[[curSp()]]$procEnvs$bgMask, shinyLogs)
      # ensure the model object was returned before proceeding
      req(mod.Corr)
      
      # LOAD INTO SPP ####
      spp[[sp]]$results <- mod.Corr
